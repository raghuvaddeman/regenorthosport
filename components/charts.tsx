  ...
  return (
    <div className="flex h-48 items-end gap-1.5 overflow-x-auto pb-1">
      {days.map((key) => {
        ...
        return (
          <div key={key} className="flex min-w-[28px] flex-1 flex-col items-center gap-1.5">
            <div className="flex h-36 w-full items-end">
              <div
                className="w-full rounded-t-sm bg-indigo-500/80 transition-all hover:bg-indigo-500"
                style={{ height: `${heightPct}%` }}
                title={`${count} call${count === 1 ? "" : "s"} on ${label(key)}`}
              />
            </div>
            <span className="text-[10px] tabular-nums text-zinc-400">{count}</span>
            <span className="whitespace-nowrap text-[9px] text-zinc-400">{label(key)}</span>
          </div>
        );
      })}
    </div>
...
const RATING_SEGMENTS = [
  ...
  { key: "unrated", label: "Not yet rated", color: "#a1a1aa" },
] as const;
...
export function RatingDonutChart({ calls }: { calls: Call[] }) {
  ...
  const dashes = RATING_SEGMENTS.map((seg) => (counts[seg.key] / total) * circumference);
    ...
    <div className="flex flex-wrap items-center gap-6">
      <svg viewBox="0 0 160 160" className="h-40 w-40 shrink-0">
        ...
        <text
          ...
          y="76"
          textAnchor="middle"
          className="fill-zinc-900 text-2xl font-semibold dark:fill-zinc-50"
        >
          {total}
        </text>
        <text x="80" y="94" textAnchor="middle" className="fill-zinc-400 text-[10px]">
          calls
        </text>
      </svg>
      <div className="space-y-2 text-sm">
        {RATING_SEGMENTS.map((seg) => (
          <div key={seg.key} className="flex items-center gap-2">
            ...
            <span className="ml-auto font-mono text-xs text-zinc-400">{counts[seg.key]}</span>
          </div>
        ))}
      </div>
    </div>
